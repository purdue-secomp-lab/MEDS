...
//
//
// RUN: %clangxx_meds -x c -DSO_NAME=f0 %s -shared -o %t-f0.so -fPIC
// RUN: %clangxx_meds -x c -DSO_NAME=f1 %s -shared -o %t-f1.so -fPIC
// RUN: %clangxx_meds -x c -DSO_NAME=f2 %s -shared -o %t-f2.so -fPIC
// RUN: %clangxx_meds %s -ldl -pthread -o %t
// RUN: %run %t 0 3
// RUN: %run %t 2 3
