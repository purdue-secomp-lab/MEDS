...
// RUN: %clangxx_meds -DBUILD_SO=1 -fPIC -shared %s -o %t-ODR-SO.so
// RUN: %clangxx_meds %s %t-ODR-SO.so -Wl,-R. -o %t-ODR-EXE
...
// RUN: %clangxx_meds -DBUILD_SO=1 -fPIC -shared %s -o %t-ODR-SO.so -DSZ=100
...
// RUN: %clangxx_meds -DBUILD_SO=1 -fPIC -shared -mllvm -asan-use-private-alias %s -o %t-ODR-SO.so -DSZ=100
// RUN: %clangxx_meds -mllvm -asan-use-private-alias %s %t-ODR-SO.so -Wl,-R. -o %t-ODR-EXE
