...
#define OPERATOR_NEW_BODY(type) \
  ...
  return __meds::meds_memalign(0, size, &stack, type);
#define OPERATOR_NEW_BODY_ALIGN(type) \
  GET_STACK_TRACE_MALLOC;\
  return __meds::meds_memalign((uptr)align, size, &stack, type);
...
#define OPERATOR_DELETE_BODY(type) \
  ...
  __meds::meds_free(ptr, &stack, type);
...
void operator delete(void *ptr, size_t size) NOEXCEPT {
  GET_STACK_TRACE_FREE;
  __meds::meds_sized_free(ptr, size, &stack, FROM_NEW);
}
CXX_OPERATOR_ATTRIBUTE
void operator delete[](void *ptr, size_t size) NOEXCEPT {
  GET_STACK_TRACE_FREE;
  __meds::meds_sized_free(ptr, size, &stack, FROM_NEW_BR);
}
CXX_OPERATOR_ATTRIBUTE
