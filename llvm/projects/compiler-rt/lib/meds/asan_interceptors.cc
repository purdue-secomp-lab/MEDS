...
INTERCEPTOR(char*, strdup, const char *s) {
  ...
  void *new_mem = __meds::meds_malloc(length + 1, &stack);
...
INTERCEPTOR(int, fork, void) {
  ...
  Vmap *fmap = __meds::getPrimary().Forkmap;
  for(Vmap::Iterator Iter = fmap->begin();
      Iter != fmap->end(); ++Iter) {
    Vpage *vpage = *((*Iter).value);
    CHECK_NE(vpage, 0);
    CHECK_NE(vpage->addr, 0);
    uptr success = internal_mprotect(reinterpret_cast<void*>(vpage->addr),
                                     page_size_, PROT_WRITE);
    CHECK_EQ(success, 0);
  }
