...
extern "C"
NOINLINE INTERFACE_ATTRIBUTE
void *__meds_stack_alloc(uptr size) {
  GET_STACK_TRACE_MALLOC;
  return __meds::meds_malloc(size, &stack);
}

extern "C"
NOINLINE INTERFACE_ATTRIBUTE
void __meds_stack_free(void *ptr) {
  GET_STACK_TRACE_FREE;
  __meds::meds_free(ptr, &stack, FROM_MALLOC);
}

extern "C"
NOINLINE INTERFACE_ATTRIBUTE
void __meds_global_alloc(void *old_addr, uptr size) {
  GET_STACK_TRACE_MALLOC;
  void *new_addr = __meds::meds_malloc(size, &stack);
  init_relocate();
  bool res = do_relocate((unsigned long)old_addr,
                         (unsigned long)new_addr,
                         (unsigned long)size);
  REAL(memcpy)(new_addr, old_addr, size);
...
extern "C"
NOINLINE INTERFACE_ATTRIBUTE
void __meds_global_free(void *ptr) {
  GET_STACK_TRACE_FREE;
  __meds::meds_free(ptr, &stack, FROM_MALLOC);
}
...
static NOINLINE void force_interface_symbols() {
  ...
  switch (fake_condition) {
    ...
    case 46: __meds_stack_alloc(0); break;
    case 47: __meds_stack_free(0); break;
    case 48: __meds_global_alloc(0, 0); break;
    case 49: __meds_global_free(0); break;
  }
  // clang-format on
}
...
static void AsanInitInternal() {
  ...
  __meds::InitializeAllocator(allocator_options);
